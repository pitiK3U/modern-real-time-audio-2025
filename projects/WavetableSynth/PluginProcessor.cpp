...
typename DSP::Parameter<FloatType>::EffectEvaluator getParameterEffect(WavetableSynthAudioProcessor::ParameterID settingParameter)
{
    if (Param::ID::WavetablePosition.compare(settingParameter) == 0) {
        return [](float previousValue, float originalValue, float valueMultiplier, DSP::DSP<float>& dsp) {
                    ...
                    auto max = Param::Ranges::WavetablePositionMax;
                    return std::fmod(std::fmod(value, max) + max, max); // Wraps value in both directions into the range [0 - n]
                };
        ...
        return [](float previousValue, float originalValue, float valueMultiplier, DSP::DSP<float>& dsp) {
            auto resoMod = valueMultiplier * dsp.getCurrentValue();
            return std::clamp(Param::Ranges::FilterResoMax * (resoMod) + previousValue, Param::Ranges::FilterResoMin, Param::Ranges::FilterResoMax);
        };
    } else if (Param::ID::WavetableVol.compare(settingParameter) == 0
            || Param::ID::OutputVol.compare(settingParameter) == 0) {
        return [](float previousValue, float originalValue, float valueMultiplier, DSP::DSP<float>& dsp) {
...
static const std::vector<mrta::ParameterInfo> paramVector
{
    ...
    { Param::ID::HistoryPlotBufferSize, Param::Name::HistoryPlotBufferSize, Param::Units::Ms, Param::Ranges::HistoryPlotBufferSizeDefault, Param::Ranges::HistoryPlotBufferSizeMin, Param::Ranges::HistoryPlotBufferSizeMax, Param::Ranges::HistoryPlotBufferSizeInc, Param::Ranges::HistoryPlotBufferSizeSkw },
